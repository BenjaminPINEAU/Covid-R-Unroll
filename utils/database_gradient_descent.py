...
def gradient_descent_database(config):
    ...
    loss_validation = 1e10 * torch.ones(1)
    ...
    # create the folder to save data during the training (in case of a crash)
    save_dir = "model_exemple/save"
    ...
    if choix_model == 'Lexp':
        model = cp_lexp.CPunfolded_Lexp(device, tau, sigma, K_layer, optimizer, scheduler, lmb_init, learning_rate)
    elif choix_model == 'LSTexp':
        model = cp_lstexp.CPunfolded_LSTexp(device, K_layer, optimizer, scheduler, lmb_init, learning_rate)
    elif choix_model == 'l':
        model = cp_l.CPunfolded(device, sigma, tau, K_layer, optimizer, scheduler, lmb_init, learning_rate)
    elif choix_model == 'OpLin':
        model = cp_oplin.CPunfolded_OpLin(device, K_layer, optimizer, scheduler, lmb_init, learning_rate)
    elif choix_model == 'OpLin_mat':
        model = cp_oplin.CPunfolded_OpLin_mat(device, T, K_layer, optimizer, scheduler, learning_rate)
    elif choix_model == 'OpLin_vect':
        model = cp_oplin.CPunfolded_OpLin_vect(device, K_layer, optimizer, learning_rate, scheduler)
    ...
    while iteration < nepoch:
        ...
        with torch.no_grad():
            ...
            if loss_value_validation_all[iteration] < loss_validation:
                best_validation_state_dict = copy.deepcopy(model.state_dict())
                loss_validation = loss_value_validation_all[iteration]
    ...
    return model, model_best_validation, out.detach().cpu().numpy(), loss_value_train_all[:iteration], loss_value_validation_all[:iteration]