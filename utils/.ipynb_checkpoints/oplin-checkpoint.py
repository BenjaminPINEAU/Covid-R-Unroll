...
def discrete_derivation_adj(Q, device = 'cpu'):
    ...
    return Q_zeros*0.25
# discrete_derivation_adj = torch.compile(discrete_derivation_adj)
...
def op_lin_adj_mat(Q, A, device = 'cpu'):
    """
    Compute the matrix product M*R
    """
    #Q = torch.tensor(Q, dtype=torch.float)
    #A = torch.tensor(A, dtype=torch.float)
    return torch.einsum('ot,bst->bso', A, Q[:, :, :-2])*0.25

def op_lin_25(R, vect_op_lin):
    """
    Compute the matrix product MR while M = multidiag(c_1, ..., c_25)
    """
    R = R.unsqueeze(1)
    y = torch.nn.functional.conv1d(R, vect_op_lin)
    return y.squeeze(1)

def op_lin_adj_25(Q, vect_op_lin):
    """
    Compute the matrix product M*R while M = multidiag(c_1, ..., c_25)
    """
    Q = Q.unsqueeze(1)
