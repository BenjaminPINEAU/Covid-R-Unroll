...
class CPunfolded_Lexp(nn.Module):
        
    """
    Define a neural network for the unfolded Chambolle-Pock algorithm 
    parameter to learn is lambda (coeff of l1 regularization), different for each layer
    """
    ...
    def __init__(self, device, tau, sigma, K_layer, choix_optimizer, scheduler = False, lmb_init = 1, learning_rate = 1e-3):
        ...
        super(CPunfolded_Lexp, self).__init__()
        self.device = device
    ...
    def forward(self, R_initial, Z_norm, PhiZ_norm):
        ...
        for k in range(self.K_layer):
            Qk = Qk + self.param_sigma * discrete_derivation(Rk_bar) - self.param_sigma*proxL1_norm(Qk/self.param_sigma + discrete_derivation(Rk_bar), torch.exp(self.param_ell[k])/self.param_sigma)
            Rk1 = self.activation_prox_DKL(Z_norm, PhiZ_norm, Rk - self.param_tau*discrete_derivation_adj(Qk, self.device), self.param_tau)
            Rk_bar = 2*Rk1 - Rk
            Rk = Rk1
